...
class RiemannWeights:
    ...
    align_mode: str = "full"
    # --- formulacao METRICA da geometria -----------------------------------
    # Quando True e uma focal e fornecida, os termos geometricos (gauss, normal,
    # metric) sao calculados sobre a superficie 3D retroprojetada, em unidades
    # fisicas. Quando False, ou quando a focal e desconhecida, cai no comportamento
    # antigo: grafico sobre coordenadas de imagem normalizadas.
    #
    # A diferenca nao e cosmetica. Numa esfera de raio 2 m a 10 m, cuja curvatura
    # verdadeira e K = 1/R^2 = 0,25, a formulacao antiga devolve valores da ordem de
    # 10. Ver riemann/geometry.py e scripts/test_geometry_metrica.py.
    usar_metrica: bool = True
    # Teto por pixel da curvatura, em 1/m^2. Com a formulacao metrica K = 1/R^2,
    # entao 5.0 corresponde a raio de curvatura de 45 cm. Vale so para estabilidade
    # em descontinuidade; nao deve cortar o corpo da distribuicao.
    gauss_clamp_metrico: float = 5.0
...
def gauss_loss_metrica(pred: torch.Tensor, target: torch.Tensor, fx: float,
                       mask: Optional[torch.Tensor] = None,
                       smooth_sigma: float = 0.5,
                       clamp_val: float = 5.0,
                       fy: Optional[float] = None) -> torch.Tensor:
    """
    Erro de curvatura Gaussiana em UNIDADES METRICAS (1/m^2).

    Usa a superficie 3D retroprojetada com os intrinsecos, em vez do grafico sobre
    coordenadas de imagem normalizadas. Ver riemann/geometry.py para a derivacao e
    scripts/test_geometry_metrica.py para a validacao contra esfera, plano e cilindro.

    Nota sobre fx != fy: quando o dataset redimensiona para quadrado a partir de uma
    imagem que nao e quadrada (o Spring e 1920x1080), as escalas horizontal e vertical
    diferem e as duas focais precisam ser passadas separadamente.
    """
    from .geometry import surface_curvatures
    kp = surface_curvatures(pred, fx=fx, fy=fy, smooth_sigma=smooth_sigma,
                            clamp_val=clamp_val)["K"]
    kt = surface_curvatures(target, fx=fx, fy=fy, smooth_sigma=smooth_sigma,
                            clamp_val=clamp_val)["K"]
    d = (kp - kt).abs()
    if mask is not None:
        return (d * mask).sum() / mask.sum().clamp(min=1.0)
...
def normal_loss_metrica(pred: torch.Tensor, target: torch.Tensor, fx: float,
                        mask: Optional[torch.Tensor] = None,
                        fy: Optional[float] = None,
                        smooth_sigma: float = 0.0) -> torch.Tensor:
    """
    Erro angular entre as normais das superficies 3D, em 1 - cos(theta).

    Diferente da normal do grafico em coordenadas de imagem, esta e a normal geometrica
    verdadeira, obtida dos tangentes da superficie retroprojetada. Em cena de alcance
    longo as duas divergem bastante, porque o termo D/f domina os tangentes.
    """
    from .geometry import surface_normals
    npred = surface_normals(pred, fx=fx, fy=fy, smooth_sigma=smooth_sigma)
    ntgt = surface_normals(target, fx=fx, fy=fy, smooth_sigma=smooth_sigma)
    d = 1.0 - (npred * ntgt).sum(dim=1, keepdim=True).clamp(-1.0, 1.0)
    if mask is not None:
        return (d * mask).sum() / mask.sum().clamp(min=1.0)
    return d.mean()
...
class RiemannianAwareLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor,
                mask: Optional[torch.Tensor] = None,
                focal: Optional[tuple] = None) -> tuple[torch.Tensor, Dict[str, float]]:
    ...
    def _forward_fp32(self, pred, target, mask, focal=None):
        w = self.w
        parts: Dict[str, torch.Tensor] = {}

        # A focal chega como (fx, fy) na resolucao de TRABALHO. Se ausente ou
        # invalida, os termos caem no comportamento antigo e o motivo fica registrado
        # em parts["metrica_ativa"], para nao passar despercebido no CSV.
        usar_m = bool(w.usar_metrica) and focal is not None
        if usar_m:
            fx, fy = focal
            fx = float(fx); fy = float(fy)
            usar_m = fx > 0 and fy > 0
        parts_meta = 1.0 if usar_m else 0.0
        ...
        if w.normal > 0:
            if usar_m:
                parts["normal"] = w.normal * self._term(
                    "normal", normal_loss_metrica(pred_aligned, target, fx, mask, fy))
            else:
        ...
        if w.gauss > 0:
            if usar_m:
                parts["gauss"] = w.gauss * self._term("gauss", gauss_loss_metrica(
                    pred_aligned, target, fx, mask, w.gauss_smooth_sigma,
                    w.gauss_clamp_metrico, fy))
            else:
        ...
        parts_f["metrica_ativa"] = parts_meta
