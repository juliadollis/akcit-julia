...
def boundary_fscore(pred: torch.Tensor, target: torch.Tensor,
                    ...
                    modo: str = "percentil", pct: float = 99.0) -> Dict[str, float]:
    """
...
def all_metrics(pred: torch.Tensor, target: torch.Tensor,
    ...
    out.update(boundary_fscore(pred, target))
    out.update(boundary_fscore_sweep(pred, target))
    return out
...
def boundary_fscore_sweep(pred: torch.Tensor, target: torch.Tensor,
    ...
    fs = []
    for t in limiares:
        m = boundary_fscore(pred, target, tolerance_px, t, modo, pct)
