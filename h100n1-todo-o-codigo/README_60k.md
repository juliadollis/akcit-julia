...
# GenRefocus DeblurNet — reprodução (AKCIT-PIXEL) · step 60.000
...
> ✅ **Este é o checkpoint recomendado.** Melhor LPIPS nas duas mesas.
>
> **Leia "Resultados" antes de usar.** Ele fica **atrás do peso oficial no
> DPDD** e **empata com o publicado no RealDOF**. Os números estão abaixo,
> medidos, sem seleção favorável.
>
> O checkpoint do step 15.500 (mínimo da `val_loss`) está em
> [`akcit-pixel/genrefocus-deblurnet-15k`](https://huggingface.co/akcit-pixel/genrefocus-deblurnet-15k)
> — **é pior**, e o porquê está explicado lá.
...
| arquivo | o que é |
|---|---|
| `deblurNet.safetensors` | o peso, step 60.000. Nome igual ao do `Inference_deblurNet.py` oficial, então o script dos autores roda sem editar nada |
| `procedencia.json` | config, step, sha256 e o run do wandb que o gerou |

**Sobre o `best`:** a validação durante o treino usa a *loss de flow matching*
com σ e ruído fixos. Ela atinge o mínimo no step 15.500 e sobe depois — o que
parecia overfit. Medido no benchmark, o step 60.000 é **10% melhor em LPIPS**.
Ou seja: a loss de denoise **não** é proxy de qualidade perceptual aqui. O
`best` está publicado para quem quiser verificar isso, não para uso.
