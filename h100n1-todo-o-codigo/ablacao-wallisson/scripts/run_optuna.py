...
def main():
    ...
    ap.add_argument("--focal", type=float, default=None,
                    help="distancia focal em PIXELS na resolucao ORIGINAL das imagens. "
                         "Necessaria para a curvatura em unidades metricas. Se omitida, "
                         "procura intrinsics.json na raiz do dataset; se nao achar, os "
                         "termos geometricos caem na formulacao antiga e a coluna "
                         "metrica_ativa do CSV sai 0.")
    ap.add_argument("--gauss-clamp-metrico", type=float, default=5.0,
                    help="teto da curvatura em 1/m^2. 5.0 = raio de curvatura de 45 cm.")
    args = ap.parse_args()
    ...
    def objective(trial):
        ...
        w = RiemannWeights(
            ...
            gauss_smooth_sigma=trial.suggest_float("gauss_smooth_sigma", 0.0, 2.0),
            # O teto da curvatura entra na busca. Calibra-lo a mao se mostrou
            # dificil: 50 cortava 40% dos pixels e 1000 deixava ruido dominar.
            # Em unidades metricas a faixa util fica entre ~1 (raio 1 m) e ~50
            # (raio 14 cm), entao deixamos a busca escolher.
            gauss_clamp_metrico=trial.suggest_float("gauss_clamp_metrico", 1.0, 50.0,
                                                    log=True),
