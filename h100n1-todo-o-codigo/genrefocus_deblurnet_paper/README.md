# GenRefocus — DeblurNet (Stage 1) · variante **cond-only (LoRA do paper)**

> 🔵 **Esta é a variante "LoRA do paper" (cond-only).** É idêntica ao projeto
> original (`genrefocus_deblurnet/`) em dados, hiperparâmetros e scripts — a
> ÚNICA diferença é ONDE o LoRA é aplicado no forward:
>
> | | LoRA aplicado em | Inferência |
> |---|---|---|
> | Projeto original | branch **main + condição** | precisa `main_adapter="deblurring"` (`infer_mainlora.py`) |
> | **Esta cópia (paper)** | **só na condição** (`adapters = [None, None, ADAPTER]`) | **oficial** `Inference_deblurNet.py` (`main_adapter=None`) |
>
> Por quê: o `deblurNet.safetensors` **oficial** é cond-only (a inferência
> oficial usa `main_adapter=None`). Treinando assim, o modelo roda **direto** na
> pipeline oficial do paper. A mudança está em `genfocus_train/backbone.py`
> (a linha do `adapters`).
>
> **Caminhos/nomes já separados do original:** cluster
> `/raid/.../projects/genrefocus_deblurnet_paper`, wandb `run_name` com sufixo
> `-condlora`, repos HF `genrefocus-deblurnet-condlora-*`. Os dois projetos
> convivem sem se sobrescrever. Sincronize com `scripts/sync_to_cluster.sh`
> (já aponta pra pasta `_paper`).
