...
class Condition(object):
    ...
    def encode(
        ...
        if isinstance(self.condition, Image.Image):
            ...
            condition_empty = Image.fromarray(
                np.zeros((H, W, 3), dtype=np.uint8), "RGB"
            )
        tokens, ids = encode_images(
            pipe, condition_empty if empty else self.condition, self.No_preprocess
        )
...
def generate(
    ...
    **params: dict,
    ...
    if TILED_DENOISE:
        ...
        cropped_height_coord = np.arange(
            0, max(1, latent_h - TILE_SIZE + 1), stride, dtype=np.int32
        )
        cropped_width_coord = np.arange(
            0, max(1, latent_w - TILE_SIZE + 1), stride, dtype=np.int32
        )
        ...
        cropped_height_coord, cropped_width_coord = np.meshgrid(
            cropped_height_coord, cropped_width_coord
        )
    ...
    def create_mapping_from_latent_ids(latent_image_ids):
        ...
        coord_to_idx[h_coords, w_coords] = torch.arange(
            len(latent_image_ids), device=latent_image_ids.device
        )
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if TILED_DENOISE:
                ...
                weight_accumulated = torch.zeros(
                    latents.shape[:2], device=device, dtype=latents.dtype
                )  # [batch, tokens]

                # Process each tile
                for tile_idx, (ch, cw) in enumerate(
                    zip(cropped_height_coord, cropped_width_coord)
                ):
                    h_end = ch + TILE_SIZE
                    w_end = cw + TILE_SIZE
                    # Extract tile
                    ...
                    tile_noise_pred = transformer_forward(
                        self.transformer,
                        image_features=[tile_latents]
                        + (tile_c_latents if use_cond else []),
                        text_features=[prompt_embeds],
                        img_ids=[fixed_latent_image_ids]
                        + (
                            [fixed_latent_image_ids, fixed_latent_image_ids]
                            if use_cond
                            else []
                        ),
                        txt_ids=[text_ids],
                        timesteps=[timestep, timestep]
                        + (c_timesteps if use_cond else []),
                    ...
                    noise_pred_accumulated[:, tile_indices] += (
                        tile_noise_pred * tile_weights.unsqueeze(0).unsqueeze(-1)
                    )
