...
from .config import TrainConfig, config_hash, write_effective_config
...
def _make_train_batch(model: nn.Module, stage: StageName, batch: dict[str, Any]):
    """Despacha para o método de treino do wrapper DeblurNet."""
    if stage == "deblur":
    ...
    while state.global_step < total_steps:
        ...
        with accelerator.accumulate(transformer_prepared):
            ...
            with accelerator.autocast():
                ...
                loss = flow_matching_loss(outputs.prediction, outputs.target)
...
def run_deblur_stage(
    ...
    dataset = build_dataset(
        ...
        runtime=DatasetRuntimeConfig(image_size=stage_cfg.image_size, train=stage_cfg.augment),
    )
...
def run_smoke_test(
    ...
    """
    print("=" * 70)
    print("SMOKE TEST — 3 steps no caminho real (FLUX + LoRA)")
    print("=" * 70)
    ...
    dataset = build_dataset(
        ...
        runtime=DatasetRuntimeConfig(image_size=stage_cfg.image_size, train=False),
    )
    if len(dataset) == 0:
        raise RuntimeError(
    ...
    for step in range(3):
        ...
        with autocast_ctx:
            outputs = model.make_train_batch(
                blurry_image=batch_on_device["blurry_image"],
                sharp_image=batch_on_device["aif_image"],
            )
            loss = flow_matching_loss(outputs.prediction, outputs.target)
    ...
    export_lora_safetensors(backbone, sf_path, stage="deblur")
