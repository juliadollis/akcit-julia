...
def main():
    ...
    # Todo braco que nao seja o controle e comparado ao controle. Antes so os B3
    # entravam aqui, e o B1 (curvatura isolada) ficava de fora justamente da
    # tabela que responde a pergunta.
    b3s = sorted(n for n in completos if not n.startswith("B0"))
    ...
    for b3 in b3s if b0 else []:
        ...
        fam3 = b3.split("_")[0]
        print(f"| metrica | {fam3} | B0 (controle) | diferenca | quem vence |")
        print("|---|---|---|---|---|")
        for chave, rotulo, sentido in METRICAS:
            ...
            vence = fam3 if ((m3 > m0) == maior_melhor) else "B0"
    ...
    # os dois tetos entre si, dentro de cada familia: mesma perda, mesma
    # configuracao, so o clamp muda.
    familias = {}
    for n in b3s:
        familias.setdefault(n.split("_")[0], []).append(n)
    for fam, membros in sorted(familias.items()):
        if len(membros) != 2:
            continue
        a, b = sorted(membros)
        print(f"## Efeito do teto em {fam}: `{a}` contra `{b}`\n")
