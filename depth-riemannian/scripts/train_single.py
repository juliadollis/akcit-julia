...
def main():
    ...
    ap.add_argument("--gauss-clamp", type=float, default=50.0,
                    help="teto por pixel da curvatura (1/m^2 no modo metrico)")
    ap.add_argument("--gauss-metrica", action="store_true",
                    help="usa curvatura metrica (retroprojecao 3D real) — requer --fx-orig")
    ap.add_argument("--fx-orig", type=float, default=None,
                    help="focal em px na resolucao ORIGINAL (mediana do prepare_spring)")
    ap.add_argument("--w-orig", type=int, default=1920,
                    help="largura original (Spring=1920), p/ reescalar focal")
    ap.add_argument("--h-orig", type=int, default=1080,
                    help="altura original (Spring=1080), p/ reescalar focal")
    ...
    ap.add_argument("--seeds", type=int, default=3,
                    help="QUANTAS seeds rodar a partir de --seed-inicio")
    ap.add_argument("--seed-inicio", type=int, default=0,
                    help="primeira seed desta chamada. Existe para ampliar um "
                         "experimento ja rodado (n=3 -> n=10) sem refazer o que "
                         "esta pronto, e para dividir as seeds entre GPUs: cada "
                         "processo pega uma faixa disjunta e escreve na MESMA "
                         "pasta. Uma seed com test_metrics.json e sempre pulada.")
    ...
    fx = fy = 0.0
    if args.gauss_metrica:
        if args.fx_orig is None:
            ap.error("--gauss-metrica requer --fx-orig")
        fx = args.fx_orig * args.size / args.w_orig
        fy = args.fx_orig * args.size / args.h_orig
        print(f"[curvatura metrica] fx={fx:.1f} fy={fy:.1f} px (size={args.size})")
    ...
    w = RiemannWeights(berhu=args.berhu, grad=args.grad, normal=args.normal,
                       ...
                       gauss_clamp=args.gauss_clamp,
                       gauss_metrica=args.gauss_metrica, fx=fx, fy=fy,
                       align_mode=args.align_mode)

    for seed in range(args.seed_inicio, args.seed_inicio + args.seeds):
        seed_dir = out / f"seed_{seed}"
        # Nunca refazer uma seed concluida: o resultado dela ja foi reportado e
        # regravar mudaria numero de relatorio por acidente.
        if (seed_dir / "test_metrics.json").exists():
            print(f"\n[seed {seed}] ja concluida em {seed_dir} — pulando.")
            continue
    ...
    # Le do DISCO, nao da memoria: as seeds podem ter sido rodadas por varios
    # processos em GPUs diferentes, cada um com uma faixa. Assim o resumo sai
    # completo em qualquer um deles, e o ultimo a terminar grava o mais completo.
    concluidas = sorted(out.glob("seed_*/test_metrics.json"),
                        key=lambda q: int(q.parent.name.split("_")[1]))
    test_absrel, test_bf, seeds_lidas = [], [], []
    for q in concluidas:
        reg = json.loads(q.read_text())
        test_absrel.append(reg.get("abs_rel", float("nan")))
        test_bf.append(reg.get("boundary_fscore", float("nan")))
        seeds_lidas.append(int(q.parent.name.split("_")[1]))
    if test_absrel:
        m_ar, lo_ar, hi_ar = bootstrap_ci(test_absrel)
        m_bf, lo_bf, hi_bf = bootstrap_ci(test_bf)
        summary = {
            "n_seeds": len(test_absrel),
            "seeds": seeds_lidas,
        ...
        print(f"\n===== TESTE (held-out) — média ± IC95% entre {len(test_absrel)} seeds "
              f"(seeds {seeds_lidas}) =====")
